...
def create_tables():
    conn = psycopg2.connect(dbname='lab10', user='tanzilya', password='Almaty250505', host='localhost', port='5432')
    cur = conn.cursor()
    ...
    """)
    # Создаём таблицу с результатами игры (если её ещё нет)
    cur.execute("""
        CREATE TABLE IF NOT EXISTS snake_game_scores (
            score_id SERIAL PRIMARY KEY,
            player_name VARCHAR(255) REFERENCES users(player_name),
            score INT NOT NULL,
            level INT NOT NULL,
